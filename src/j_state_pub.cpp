    ...
    ui(new Ui::j_state_pub)
{
    ...
    QDateTime current_datetime = QDateTime::currentDateTime();//获取系统现在的时间
    QString logtime_string =current_datetime .toString("yyyy-MM-dd-hh-mm-ss");

    dat_log = new QFile("/home/robot/dat_log" + logtime_string);
...
void j_state_pub::timer_out(void){
    ...
    if(handle_data.right.enable_key){
        if(cbox[0]->isChecked())
        {joint_state.position[0] = dh_angle[0];s.setNum((float)(dh_angle[0]));le_dhangle[2]->setText(s);}//show the actual command data that was sent to slave joint
        if(cbox[1]->isChecked())
        {joint_state.position[1] = dh_angle[1];s.setNum((float)(dh_angle[1]));le_dhangle[3]->setText(s);}
        if(cbox[2]->isChecked())
        {joint_state.position[2] = -dh_angle[2];s.setNum((float)(dh_angle[2]));le_dhangle[4]->setText(s);}
        if(cbox[3]->isChecked())
        {joint_state.position[3] = -dh_angle[3];s.setNum((float)(dh_angle[3]));le_dhangle[5]->setText(s);}
        if(cbox[4]->isChecked())
        {joint_state.position[4] = dh_angle[4];s.setNum((float)(dh_angle[4]));le_dhangle[6]->setText(s);}
        if(cbox[5]->isChecked())
...
*/
    ...
    if(handle_data.left.enable_key){
        if(cbox[7]->isChecked())
        {joint_state.position[7] = -dh_angle[8];s.setNum((float)(dh_angle[8]));le_dhangle[11]->setText(s);}
        if(cbox[8]->isChecked())
        {joint_state.position[8] = dh_angle[9];s.setNum((float)(dh_angle[9]));le_dhangle[12]->setText(s);}
        if(cbox[9]->isChecked())
        {joint_state.position[9] = -dh_angle[10];s.setNum((float)(dh_angle[10]));le_dhangle[13]->setText(s);}
        if(cbox[10]->isChecked())
        {joint_state.position[10] = dh_angle[11];s.setNum((float)(dh_angle[11]));le_dhangle[14]->setText(s);}
        if(cbox[11]->isChecked())
        {joint_state.position[11] = dh_angle[12];s.setNum((float)(dh_angle[12]));le_dhangle[15]->setText(s);}
        if(cbox[12]->isChecked())
        {joint_state.position[12] = dh_angle[13];s.setNum((float)(dh_angle[13]));le_dhangle[16]->setText(s);}
        if(cbox[13]->isChecked())
        {joint_state.position[13] = -dh_angle[14];s.setNum((float)(dh_angle[14]));le_dhangle[17]->setText(s);}
...
void j_state_pub::setup_ui(){
    ...
    // add timedelay and pack loss
    QHBoxLayout * test_components_layout = new QHBoxLayout();
    QLabel *tmp_lbl1 = new QLabel(this);

    delay_spinbox = new QDoubleSpinBox(this);
    //delay_spinbox->setFixedWidth(50);
    delay_spinbox->setMaximum(2.0);
    delay_spinbox->setMinimum(0.0);
    delay_spinbox->setDecimals(1);
    delay_spinbox->setSingleStep(0.1);
    tmp_lbl1->setText(tr("Time Delay(s)"));


    QLabel *tmp_lbl2 = new QLabel(this);
    loss_spinbox = new QDoubleSpinBox(this);
    tmp_lbl2->setText(tr("Package Loss(%)"));
    loss_spinbox->setMaximum(0.4);
    loss_spinbox->setMinimum(0.0);
    loss_spinbox->setDecimals(1);
    loss_spinbox->setSingleStep(0.1);

    test_components_layout->addWidget(tmp_lbl1);
    test_components_layout->addWidget(delay_spinbox);
    test_components_layout->addStretch();
    test_components_layout->addWidget(tmp_lbl2);
    test_components_layout->addWidget(loss_spinbox);
    test_components_layout->addStretch();
    ...
    vLayout[3]->addLayout(test_components_layout);
